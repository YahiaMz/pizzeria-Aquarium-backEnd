...
export class CartsService {
  ...
  async isThisFoodExistInThisCart(
    user_Id: number,
    food_Id: number,
    size_Id: number,
  ) {
  ...
  async addToCart(addProductToCartDto: AddFoodToCartDto) {
    ...
    if (addProductToCartDto.size_Id != null) {
      foodSize = await this.foodSizeService.findSizeByIdOrThrowException(
        addProductToCartDto.size_Id,
      );
      if (foodSize && foodSize.food != addProductToCartDto.food_Id) {
        MyExceptions.throwException(
          'food size does not match this cart food item',
          null,
        );
      }
    } else {
    ...
    let item = await this.isThisFoodExistInThisCart(
      +user.id,
      +food.id,
      addProductToCartDto.size_Id,
    );
  ...
  async addThisQuantityToCartItem(itemId: number, newQuantity: number) {
    ...
    if (cartItem.quantity + newQuantity <= 0) {
      MyExceptions.throwException(
        'wrong operation',
        'you cant change quantity until <= 0',
      );
    }
    ...
    try {
      ...
      MyExceptions.throwException(
        'something wrong while changing the quantity',
        error.message,
      );
    }
  }

  async changeCartItemSize(user_Id: number, itemId: number, size_Id: number) {
    let cartItem = await this.findCartAndItFoodIdByIdOrThrowException(itemId);
    let newSize = await this.foodSizeService.findSizeByIdOrThrowException(
      size_Id,
    );
    ...
    if (+cartItem.food !== +cartItem.food) {
      MyExceptions.throwException(
        'this food size is not a size of this food',
        null,
      );
    }

    let isExist = await this.isThisFoodExistInThisCart(
      user_Id,
     +cartItem.food,
      size_Id,
    );
    ...
    try {
      if (isExist && isExist.id != cartItem.id) {

        isExist.quantity += +cartItem.quantity;
        await this.cartRepository.remove(cartItem);
        let updatedSize = (await this.cartRepository.save(isExist));
        return {
          id: -1,
          size: "refresh" ,
          price: 0,
      ...
      MyExceptions.throwException(
        'something wrong while changing the size',
        error.message,
      );
    }
  }
